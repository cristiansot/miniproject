const convertCurrency = require('./currency_utils');
...
test('same currency conversion', () => {
  const result = convertCurrency(cdnCurrency.conversionRate, cdnCurrency.conversionRate, 100);
...
test('CDN to GBP conversion', () => {
  const result = convertCurrency(cdnCurrency.conversionRate, gbpCurrency.conversionRate, 100);
...
test('CDN to USD conversion', () => {
  const result = convertCurrency(cdnCurrency.conversionRate, usdCurrency.conversionRate, 100);
...
test('USD to GBP conversion', () => {
  const result = convertCurrency(usdCurrency.conversionRate, gbpCurrency.conversionRate, 200)
...
test('GBP to CDN conversion', () => {
  const result = convertCurrency(gbpCurrency.conversionRate, cdnCurrency.conversionRate, 50)
